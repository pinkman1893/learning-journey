...
> 学习资料：
>
> - [x] [图解 MoE](https://www.maartengrootendorst.com/blog/moe/)【非常好的文章】
...
> - [x] [L 站：【模型训练】MoE 相关知识点汇总与讲解（下）](https://linux.do/t/topic/2687837)
>
...
## MoE 两大核心组件
### 专家 Expert
专家并不指精通于某一特定领域（计算机、生物学等），MoE 中的专家擅长处理的是句法结构等细粒度特征，故 MoE 专家的专长在于处理特定上下文中的特定 Token
...
> MoE 的原始路由思想是 Token Choice，即让新加入的 Token 选择一个或多个最匹配的专家。

#### 核心思想
反过来让各个 Expert 选择 Top-c 个 Token，使其天然具备负载均衡特性

#### 原理实现
每个 Expert 看所有 Token，再按照 Top-c 的 c 值来选择自己擅长的 Token

+ 若某个 Token 被多次选择，其输出即为多个 Expert 的加权合并
+ 若某个 Token 没有 Expert 去选，直接通过残差连接路径进入下一层，但会降低模型性能

#### 缺陷
+ Token 可能被选很多次，也可能不会被选，模型性能表现不好
+ 最大缺陷：破坏自回归模型的因果性。Expert 在训练时一次性看到完整语句，但在推理时是逐 Token 生成，造成训练与推理不匹配，严重影响模型能力
...
### Total & Activated Parameters 总参数与激活参数
+ 总参数指模型中实际存储的全部参数，包括 Embedding、Attention、Router、Expert、输出层等
    - $总参数 = 共享参数 + Expert 的全部参数$
+ 激活参数指处理一个 Token 时，实际参与 Forward 前向计算的参数
    - $激活参数 = 共享参数 + k 个被激活的 Expert 的参数和$

---

+ MoE 模型的显存需求接近其总参数量，因为模型的总参数需要加载到显存中；但单个 Token 计算时，只需要激活参数参与即可。因此，MoE 模型单 Token 的计算量远低于同总参数量规模的稠密模型

### Mixtral 8×7B 模型参数分析
> Mixtral 8×7B 是 Mistral AI 发布的一款稀疏混合专家模型。
>
> Mixtral 有 8 个 Expert，使用 Top-2 Routing。

![](assets/280085522/3b22cba2d08b.png)

$$
总参数 = Emb + Att + Rou + \textbf{total}\ Expert + Head = 46.7B
$$

$$
激活参数 = Emb + Att + Rou + 2 \times \textbf{single}\ Expert + Head = 12.8B
$$

因此使用该模型时，需要加载 46.7B 参数，推理时只需要使用 12.8B

## MoE 变体
### Shared MoE 共享混合专家
核心思想：共享专家负责存储一些最基础、重复、通用的能力等，每次 Token 计算都会激活；而路由专家则专注于学习更专业的能力，由 Router 按需选择

### Fine-Grained Experts 细粒度专家
核心思想：把大 Expert 拆成更多、更小的 Expert，每个 Token 选择更多的小 Expert 进行处理计算；分工更细，组合方式更多

### Latent MoE 潜在混合专家
核心思想：先把 Token 的隐藏向量压缩到更低的维度，专家在低维空间中处理，再升维还原回原维度；由此降低计算成本和预算压力
...
| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-08-04 | 首次发布 | 发布尚在整理中的阶段性版本，记录 MoE 架构、路由与负载均衡机制 |
| 2026-08-05 | 内容更新 | 完成笔记，补充 Expert Choice Routing、参数与计算需求、Mixtral 案例及 MoE 变体 |
