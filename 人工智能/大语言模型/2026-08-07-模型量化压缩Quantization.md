...
先把模型正常训练完，然后在训练模型之后对模型的参数（包括权重和激活值）进行量化。

- 权重量化采用对称或非对称量化
- 激活值量化有两种形式：动态量化与静态量化

#### 动态量化 dynamic quantization

**核心思想：** 在正式推理的每一层隐藏层得到激活值后，现场根据激活值的分布计算量化参数 $`s`$ 和 $`z`$；不同层拥有不同量化参数。

![](assets/280475462/baef25ed00eb.png)

![](assets/280475462/0938d3f8be85.png)

#### 静态量化 static quantization

**核心思想：** 使用一个校准数据集输入模型，根据分布提前计算量化参数，在推理期间直接执行量化。

![](assets/280475462/2a0587397770.png)

---

- 动态量化更精准灵活，但会增加推理计算时间
- 静态量化更快速，但精度较低，量化误差较大

#### 训练后量化常见方法

- **GPTQ：** 一种常见的低 bit 训练后权重量化方法，会逐层对权重进行量化，并通过误差估计与权重调整来进行量化补偿，以降低量化带来的性能损失
- **GGUF：** 一种常用于本地大模型部署的模型文件格式，可保存多种量化类型，并常配合 llama.cpp 实现 CPU/GPU 混合推理；其量化实现通常采用分块方式，为不同块使用独立的缩放参数

### QAT（Quantization Aware Training，量化感知训练）

#### 核心概念

PTQ 训练后量化的核心缺点为，未考虑模型实际的训练过程。

QAT 量化感知训练旨在训练期间学习量化过程，因而比 PTQ 更精确。

#### Fake Quantization 伪量化

QAT 在模型的训练期间引入了伪量化；指在层间将权重量化为 INT4 等类型，再反量化回 FP32。

![](assets/280475462/4d0c5bf5369c.png)

QAT 通过在训练中探索量化损失的 wide minima 宽极小值，以最小化量化误差（narrow minima 窄极小值往往会导致更大的量化误差）。

![](assets/280475462/79fcac4dfa88.png)

宽极小值和窄极小值解释如下：

![](assets/280475462/fec4e3e97bf5.png)

![](assets/280475462/28a84af111ef.png)

## 拓展知识：极低位宽模型 BitNet

### 概念与核心思想

**BitNet** 是一种面向极低位宽设计的模型思路，从**训练**阶段就让模型适应超低位宽权重（1-bit/1.58-bit）。

- **BitNet：** 权重主要限制为 `-1 / +1`
- **BitNet 1.58-bit：** 权重限制为 `-1 / 0 / +1`（三种状态理论信息量约为 `log₂3 ≈ 1.58 bit`）
...
| 2026-08-20 | 内容更新 | 完成 PTQ、QAT 与 BitNet 内容，补充动态/静态量化、GPTQ、GGUF 和伪量化等知识 |
